#!/bin/bash
# Build the 4 sweep configs into isolated tmp dirs with INTERLEAVE_HT=ON,
...
MAP=/tank/alexb/swole/db-engines/build_interleave_map.txt
: > "$MAP"
...
build_one() {
  ...
  ( cd "$DIR"
    cmake ../.. $opts -DINTERLEAVE_HT=ON > cmake.log 2>&1
    make -j 22 run_tpch > make.log 2>&1
    {
      echo "# build-tpch provenance ($name)"
      echo "# built:       $(date -u +%Y-%m-%dT%H:%M:%SZ)"
      echo "# cmake opts:  $opts -DINTERLEAVE_HT=ON"
      echo "# configure:   cmake ../.. $opts -DINTERLEAVE_HT=ON"
  ...
  # making it knowable at submit time for the ingest collector. INTERLEAVE_HT
  # is confirmed with the user before long sf100 runs and recorded explicitly.
  local BE
  BE=$(register-build --binary "$BIN" --config "$name" \
                      --define INTERLEAVE_HT=ON --buildcmds "$DIR/buildcmds.txt")
