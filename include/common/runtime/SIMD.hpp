...
// --- Architecture Detection & SIMD Mapping ---
    ...
    // --- Foundation-Safe Polyfills (for KNL etc.) ---
    ...
    // --- Polyfills for intrinsics missing in older SIMDe builds ---
    ...
    // Conversion: unsigned 32->64
    #if !defined(_mm512_cvtepu32_epi64)
    static inline simde__m512i simde_mm512_cvtepu32_epi64_polyfill(simde__m256i a) {
    ...
    #define _mm512_cvtepu32_epi64(a) simde_mm512_cvtepu32_epi64_polyfill(a)
    #endif

    // Conversion: signed 32->64
    #if !defined(_mm512_cvtepi32_epi64)
    static inline simde__m512i simde_mm512_cvtepi32_epi64_polyfill(simde__m256i a) {
        simde__m128i lo = simde_mm256_castsi256_si128(a);
        simde__m128i hi = simde_mm256_extracti128_si256(a, 1);
    ...
    // Gather: i32 indices -> 32-bit values
    #ifndef _mm512_i32gather_epi32
    static inline simde__m512i _mm512_i32gather_epi32_polyfill(simde__m512i idxs, void const* base_addr, int scale) {
        ...
        switch (scale) {
            case 1:  res_lo = simde_mm256_i32gather_epi32((int const*)base_addr, idx_lo, 1);
                     res_hi = simde_mm256_i32gather_epi32((int const*)base_addr, idx_hi, 1); break;
            case 2:  res_lo = simde_mm256_i32gather_epi32((int const*)base_addr, idx_lo, 2);
                     res_hi = simde_mm256_i32gather_epi32((int const*)base_addr, idx_hi, 2); break;
            case 4:  res_lo = simde_mm256_i32gather_epi32((int const*)base_addr, idx_lo, 4);
                     res_hi = simde_mm256_i32gather_epi32((int const*)base_addr, idx_hi, 4); break;
            case 8:  res_lo = simde_mm256_i32gather_epi32((int const*)base_addr, idx_lo, 8);
                     res_hi = simde_mm256_i32gather_epi32((int const*)base_addr, idx_hi, 8); break;
            default: res_lo = simde_mm256_setzero_si256();
                     res_hi = simde_mm256_setzero_si256(); break;
        }
        simde__m512i res = simde_mm512_setzero_si512();
    ...
    #define _mm512_i32gather_epi32(idx, base, scale) _mm512_i32gather_epi32_polyfill(idx, base, scale)
    #endif
...
struct Vec8u {
   ...
   Vec8u(uint64_t x0, uint64_t x1, uint64_t x2, uint64_t x3,
...
#else
   vec_reg_t a_hi = _mm512_srli_epi64(a.reg, 32);
   vec_reg_t b_hi = _mm512_srli_epi64(b.reg, 32);
   vec_reg_t m_lo = _mm512_mul_epu32(a.reg, b.reg);
   vec_reg_t m_ahi = _mm512_mul_epu32(a_hi, b.reg);
   vec_reg_t m_bhi = _mm512_mul_epu32(a.reg, b_hi);
   vec_reg_t m_hi = _mm512_add_epi64(m_ahi, m_bhi);
   return _mm512_add_epi64(m_lo, _mm512_slli_epi64(m_hi, 32));
#endif
}
...
// Comparisons use _mm512_cmp_epi64_mask with predicates (portable across SIMDE)
...
struct Vec16u {
   ...
   Vec16u(uint32_t x0, uint32_t x1, uint32_t x2, uint32_t x3, uint32_t x4,
          ...
          uint32_t x15) {
      reg = _mm512_set_epi32(x0, x1, x2, x3, x4, x5, x6, x7,
...
// Comparisons use _mm512_cmp_epi32_mask with predicates (portable across SIMDE)
